...
import RBAC, { RBACProvider } from './RBAC';

// ─── CONSTANTS ───────────────────────────────────────────────────────────────
const API = "https://aura-school-backend.onrender.com";
const PIN = 'shyam232003';
...
// ─── ROOT APP — REPLACE THIS ENTIRE FUNCTION ONLY ────────────────────────────
// Everything above (Badge, Card, Stat, Input, Select, Btn, AIInsightBox,
// computeScore, nextAction, CommandCenter, AdmissionsEngine, FeesEngine,
// StudentsModule, AutomationEngine, CommsSystem, InsightsModule, SettingsModule)
// stays exactly as you had it. Only replace the export default function App().

export default function App() {

  // ── FIX 1: Read auth synchronously. No setTimeout anywhere. ──────────────
  // We use a function initialiser so it only runs once on mount.
  ...
  // ── FIX 2: Store the Firestore unsubscribe in a ref (survives re-renders). ──
  // Never store it in state — that causes re-render loops.
  const unsubRef = useRef(null);

  // ── FIX 3: One function that starts ALL data listeners. ─────────────────
  // We call it from useEffect AND immediately after login.
  const startDataSync = useCallback(() => {
    // Kill any existing listener before starting a new one (prevents duplicates)
    if (unsubRef.current) {
      unsubRef.current();
      unsubRef.current = null;
    }

    // Firestore real-time leads
    ...
    // One-time fees fetch (add polling here if you need live updates)
    axios.get(`${API}/get-real-fees`)
      .then(r => setRealFees(r.data))
      .catch(err => console.error('Fees fetch error:', err));
  }, []); // no deps — safe to call anytime

  // ── FIX 4: On mount, if already authed (page refresh), start sync immediately.
  // The key insight: useEffect with [authed] re-runs when authed flips true,
  // but on a hard refresh authed is ALREADY true from localStorage,
  // so the effect runs once on mount — which is exactly what we want.
  useEffect(() => {
    if (authed) {
      startDataSync();
    }
    // Cleanup: unsubscribe when component unmounts (tab close, etc.)
    return () => {
      if (unsubRef.current) unsubRef.current();
    };
  }, [authed, startDataSync]);
  // Note: startDataSync is memoised with useCallback so this is safe.

  // ── FIX 5: Login — set state directly, no setTimeout ────────────────────
  const handleLogin = () => {
    if (pin === PIN) {
      try { localStorage.setItem('sd_crm_auth', '1'); } catch { }
      setPinError('');
      setAuthed(true);
      // useEffect above will see authed=true and call startDataSync()
    } else {
  ...
  // ── FIX 6: Logout — stop listener BEFORE clearing state ──────────────────
  const handleLogout = () => {
    // 1. Kill Firebase listener immediately
    if (unsubRef.current) {
      unsubRef.current();
      unsubRef.current = null;
    }
    // 2. Clear persisted auth
    try { localStorage.removeItem('sd_crm_auth'); } catch { }
    // 3. Clear in-memory data so it doesn't flash on next login
    setLeads([]);
    setRealFees([]);
    setPin('');
    setPinError('');
    // 4. Finally flip authed — this triggers a re-render to the login screen
    setAuthed(false);
  };
  ...
  if (!authed) {
    return (
      <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0d1117', fontFamily: "'Segoe UI', sans-serif" }}>
        <div style={{
          background: '#111827',
          border: `1px solid ${pinError ? '#f45b69' : '#1f2d3d'}`,
          borderRadius: 24, padding: '48px 40px', textAlign: 'center', width: 360,
          ...
          <input
            ...
            style={{
              width: '100%', padding: '14px', textAlign: 'center', fontSize: 22, letterSpacing: 8,
              background: '#0d1117', border: `1px solid ${pinError ? '#f45b69' : '#1f2d3d'}`,
              borderRadius: 12, color: '#f1f5f9', outline: 'none', boxSizing: 'border-box', marginBottom: 10
            }}
          />
          {pinError && <p style={{ color: '#f45b69', fontSize: 12, marginBottom: 10, fontWeight: 700 }}>❌ {pinError}</p>}
          <Btn style={{ width: '100%', padding: '14px', marginTop: 6 }} onClick={handleLogin}>🔓 Unlock System</Btn>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0d1117', fontFamily: "'Segoe UI', sans-serif", color: '#f1f5f9' }}>
          ...
          {TABS.map(tab => {
            ...
            return (
              ...
              >
                ...
                {hasBadge && (
                  <span style={{ marginLeft: 'auto', background: '#f45b69', color: '#fff', borderRadius: 20, padding: '1px 7px', fontSize: 10, fontWeight: 800 }}>
                    {activeLeadsCount}
                  </span>
                )}
              </div>
      ...
      <div style={{ marginLeft: sidebarOpen ? 240 : 68, flex: 1, padding: '32px', transition: 'margin .25s', minHeight: '100vh' }}>
        ...
        {activeTab === 'insights' && <><InsightsModule leads={leads} /><RealtimeDashboard /></>}
        {activeTab === 'students' && <><StudentsModule leads={leads} /><MultiApp /></>}
        {activeTab === 'command' && <><CommandCenter leads={leads} onSendCampaign={sendCampaign} /><AIBrainDashboard /></>}
        {activeTab === 'documents' && <DocsManager userRole="admin" schoolId="AuraIndore_01" />}
        {activeTab === 'admissions' && <AdmissionsEngine leads={leads} />}
        {activeTab === 'fees' && <FeesEngine leads={leads} realFees={realFees} />}
        {activeTab === 'students' && <StudentsModule leads={leads} />}
        {activeTab === 'automation' && <AutomationEngine leads={leads} />}
        {activeTab === 'comms' && <CommsSystem leads={leads} />}
        {activeTab === 'insights' && <InsightsModule leads={leads} />}
        {activeTab === 'settings' && <SettingsModule />}
        {activeTab === 'timetable' && <TimetableOptimizer />}
        {activeTab === 'transport' && <TransportApp />}
        {activeTab === 'marketplace' && <ModularMarketplace />}
